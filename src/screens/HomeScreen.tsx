...
const genres = [['sertanejo', 'Sertanejo'], ['pagode_samba', 'Pagode e samba'], ['funk', 'Funk'], ['trap_rap', 'Rap e trap'], ['pop', 'Pop'], ['axe', 'Axé'], ['forro', 'Forró'], ['mpb', 'MPB'], ['rock', 'Rock'], ['paredao_baiano', 'Paredão baiano']];
export function HomeScreen({ profile, start, ranking, resume, resumePreferences }: { profile: Profile | null; start: (name: string, preferences: MusicPreferences) => void; ranking: () => void; resume?: () => void; resumePreferences?: MusicPreferences }) {
  ...
  const [preferences, setPreferences] = useState<MusicPreferences>(() => {
    try {
      ...
      if (saved && Array.isArray(saved.genres) && saved.genres.length <= genres.length && saved.genres.every((g: unknown) => genres.some(([id]) => id === g)) &&
        (saved.yearFrom === null || (Number.isInteger(saved.yearFrom) && saved.yearFrom >= 1900 && saved.yearFrom <= currentYear)) && (saved.yearTo === null || (Number.isInteger(saved.yearTo) && saved.yearTo >= 1900 && saved.yearTo <= currentYear))) { if (saved.yearFrom === null || saved.yearTo === null || saved.yearFrom <= saved.yearTo) return saved; }
    } catch { /* Preferences are optional. */ }
    return { genres: [], yearFrom: 2015, yearTo: currentYear };
  });
  const resumeMatches = resumePreferences && preferences.yearFrom === resumePreferences.yearFrom && preferences.yearTo === resumePreferences.yearTo && [...new Set(preferences.genres)].sort().join(',') === [...new Set(resumePreferences.genres)].sort().join(',');
  const invalidYears = [preferences.yearFrom, preferences.yearTo].some(year => year !== null && (!Number.isInteger(year) || year < 1900 || year > currentYear)) || (preferences.yearFrom !== null && preferences.yearTo !== null && preferences.yearFrom > preferences.yearTo);
  ...
  return <div className="screen home-screen">
    ...
    <h1>Reconhece só<br />por um <span className="gradient-text">trechinho?</span></h1>
    ...
    <form onSubmit={event => { event.preventDefault(); if (invalidYears || matchLoading || (matching !== null && matching.titles < 10)) return; start(profile?.displayName ?? (name.trim() || 'Jogador'), preferences); }} className="home-form">
      <fieldset className="music-preferences"><legend><Headphones size={18} aria-hidden="true" /> Seu gosto musical</legend>
        ...
        <button className="text-button" type="button" onClick={() => updatePreferences({ ...preferences, yearFrom: 2020, yearTo: currentYear })}>De 2020 para cá</button>
        ...
        <div className="matching-catalog" role="status" aria-live="polite"><Music2 size={18} aria-hidden="true" />
          {invalidYears ? 'Informe anos entre 1900 e ' + currentYear + ', com o início anterior ou igual ao fim.' : matchLoading ? 'Conferindo sua seleção…' : matchError ? 'Não foi possível contar agora. Você ainda pode tentar iniciar.' : matching ? matching.songs.toLocaleString('pt-BR') + ' músicas de ' + matching.artists + ' artistas combinam com você' : 'Escolha sua mistura.'}
        </div>
      ...
      {resume && <>
        <p role="status">{resumePreferences ? 'Partida salva: ' + (resumePreferences.yearFrom ?? 'qualquer início') + ' até ' + (resumePreferences.yearTo ?? 'qualquer ano') + '. ' + (resumeMatches ? 'Os filtros são os mesmos da seleção acima.' : 'Você mudou os filtros. Comece uma nova partida para aplicá-los.') : 'Conferindo os filtros da partida salva…'}</p>
        <button type="button" className="secondary-button" disabled={!resumeMatches} onClick={resume}>Continuar partida salva</button>
      </>}
      <button className="primary-button" type="submit" disabled={invalidYears || matchLoading || (matching !== null && matching.titles < 10)}>{resume ? 'Começar nova partida' : 'Bora jogar'}<ArrowRight size={19} /></button>
